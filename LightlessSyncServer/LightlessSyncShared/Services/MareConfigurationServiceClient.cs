...
public class LightlessConfigurationServiceClient<T> : IHostedService, IConfigurationService<T> where T : class, ILightlessConfiguration
{
    ...
    private readonly ILogger<LightlessConfigurationServiceClient<T>> _logger;
    ...
    private Uri GetRoute(string key, string value)
    {
        if (_config.CurrentValue.GetType() == typeof(ServerConfiguration))
            return new Uri((_config.CurrentValue as ServerConfiguration).MainServerAddress, $"configuration/LightlessServerConfiguration/{nameof(LightlessServerConfigurationController.GetConfigurationEntry)}?key={key}&defaultValue={value}");
        if (_config.CurrentValue.GetType() == typeof(LightlessConfigurationBase))
            return new Uri((_config.CurrentValue as LightlessConfigurationBase).MainServerAddress, $"configuration/LightlessBaseConfiguration/{nameof(LightlessBaseConfigurationController.GetConfigurationEntry)}?key={key}&defaultValue={value}");
        if (_config.CurrentValue.GetType() == typeof(ServicesConfiguration))
            return new Uri((_config.CurrentValue as ServicesConfiguration).MainServerAddress, $"configuration/LightlessServicesConfiguration/{nameof(LightlessServicesConfigurationController.GetConfigurationEntry)}?key={key}&defaultValue={value}");
        if (_config.CurrentValue.GetType() == typeof(StaticFilesServerConfiguration))
            return new Uri((_config.CurrentValue as StaticFilesServerConfiguration).MainFileServerAddress, $"configuration/LightlessStaticFilesServerConfiguration/{nameof(LightlessStaticFilesServerConfigurationController.GetConfigurationEntry)}?key={key}&defaultValue={value}");

        throw new NotSupportedException("Config is not supported to be gotten remotely");
    }

    public LightlessConfigurationServiceClient(ILogger<LightlessConfigurationServiceClient<T>> logger, IOptionsMonitor<T> config, ServerTokenGenerator serverTokenGenerator)
    {
        _config = config;
        _logger = logger;
    ...
    {
        _logger.LogInformation("Starting LightlessConfigurationServiceClient");
